...
// define the
router.get(
  "/firstAddress=:firstAddress/secondAddress=:secondAddress/activity=:activity/transportation=:transportation",
  setResponseHeaders,
  validateParams,
  validateTransportationFeasibility,
  async (req, res) => {
    const { activity } = req.params;
    const maxNumBusinessesToReturn = 10;
    const googleAPIResponse = res.locals.directionsResponseData;
    const addressesOfGeographicCoordinates =
      res.locals.addressesOfGeographicCoordinates;
    const responseBody = {
      message: "Valid request!",
      requestBody: { ...req.params, addressesOfGeographicCoordinates },
      isValid: true,
    };
    ...
    try {
      ...
      const businessResponse = await fetchBusinesses(
        activity,
      ...
      if (businessData.data.search.business.length < maxNumBusinessesToReturn) {
        ...
        const nearbyCitiesResponse = await fetchNearbyCities(
          midPointGeographicCoordinate
        );
        const nearbyCitiesData = nearbyCitiesResponse.data.data;

        // Fetch businesses from nearby cities
        const numBusinessesLeft =
          maxNumBusinessesToReturn - businessData.data.search.business.length;
        responseBody["nearbyCitiesAndBusinesses"] =
          await findBusinessesFromNearbyCities(
            activity,
            nearbyCitiesData,
            maxNumBusinessesToReturn,
            numBusinessesLeft,
