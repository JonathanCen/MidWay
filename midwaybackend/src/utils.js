...
const calculateMidPoint = (googleAPIResponse) => {
  ...
  while (
    ...
    estimatedMidPointTravelDuration
  ) {
    currentDuration += steps[stepIndex].duration.value;
    stepIndex += 1;
  }
  ...
  const startingLocation = steps[stepIndex].start_location,
    endingLocation = steps[stepIndex].end_location;
  const geographicPathCoordinates = [startingLocation].concat(
    util.decodePath(steps[stepIndex].polyline.points),
    [endingLocation]
  );

  // Calculate the rate of duration/point and the left over duration from the midpoint
  const durationForEachCoordinate =
    steps[stepIndex].duration.value / geographicPathCoordinates.length;
  const leftOverDuration = estimatedMidPointTravelDuration - currentDuration;

  // Compute the midpoint coordinate index
  const midPointCoordinateIndex =
    leftOverDuration > 0
      ? Math.floor(leftOverDuration / durationForEachCoordinate) - 1
      : 0;

  // Compute the midpoint
  const midPointCoordinate = {
    lat:
      (geographicPathCoordinates[midPointCoordinateIndex].lat +
        geographicPathCoordinates[midPointCoordinateIndex + 1].lat) /
      2,
    lng:
      (geographicPathCoordinates[midPointCoordinateIndex].lng +
        geographicPathCoordinates[midPointCoordinateIndex + 1].lng) /
      2,
  };
...
const fetchBusinesses = (activity, geographicCoordinates, offset = 5) => {
  ...
  const { lat, lng } = geographicCoordinates,
    searchRadius = 40000,
    searchActivity = activity === "Any" ? "Fun" : activity;

  // graphQL query based on the provided inputa
  const graphQLQuery = `
      query {
        search(term: "${searchActivity}",
                latitude: ${lat},
                longitude: ${lng}
                limit: ${offset}) {
            business {
 ...
 */
const findBusinessesFromNearbyCities = async (
  activity,
  nearbyCitiesData,
  maxNumBusinessesToReturn,
  numBusinessesLeft,
  seenBusinesses
) => {
  try {
    // Return an array of objects (cities)
    let nearbyCitiesAndBusinesses = [],
      citiesIndex = 0;
    ...
    while (citiesIndex < nearbyCitiesData.length && numBusinessesLeft > 0) {
      ...
      const numBusinessesToFetch =
        (maxNumBusinessesToReturn - numBusinessesLeft) * 2;
      const cityGeographicCoordinates = {
        lat: nearbyCitiesData[citiesIndex].latitude,
        lng: nearbyCitiesData[citiesIndex].longitude,
      };
      const cityBusinessesResponse = await fetchBusinesses(
        activity,
        cityGeographicCoordinates,
        numBusinessesToFetch
      );
...
module.exports = {
  ...
  fetchBusinesses,
  fetchNearbyCities,
  createSetOfSeenBusinesses,
  findBusinessesFromNearbyCities,
};
