  ...
  return (
    <List sx={{ width: "100%", bgcolor: 'background.paper' }}>
      <ListItem alignItems="flex-start">
        ...
        <ListItemText
          primary={<a className="business-information-reviewer-profile-url" href={userProfileURL} target="_blank">{userName}</a>}
          secondary={
            <React.Fragment>
              <Typography
                sx={{ display: 'inline' }}
                ...
                color="text.primary"
              >
                {`Rating: `} {<StyledRating name="business-information-reviewer-rating" value={rating} precision={0.5} icon={<StarsIcon fontSize="inherit" />} 
          emptyIcon={<StarsIcon fontSize="inherit" />} readOnly size="small" />}
              </Typography>
              {` — ${review.text} `}
              {<a href={reviewURL} className="business-information-review-url" target="_blank">Read More</a>} 
            </React.Fragment>
          }
        />
      </ListItem>
