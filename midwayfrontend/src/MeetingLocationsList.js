  ...
  return (
    <Paper
      ...
      sx={{
        ...
        <div className="meeting-location-list-title"> {nearbyCity !== undefined ? `${activity} Places Near Midpoint, ${nearbyCity}` : `${activity} Places Near Midpoint`} </div>
        {meetingLocationsData.map((business, index) => 
          <MeetingLocationsListBusiness business={business} num={businessNumbering[business.name]} key={index}/>
        )}
    </Paper>
  )
};
